...
public class SuddenDeathStatusCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        try {
            if (args.length == 0) {
                if (!requirePermission(sender, PERMISSION_ADMIN)) return true;
                if (requirePlayer(sender)) sendHelpMessage(sender);
                return true;
            }
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "status" -> {
                    if (!requirePermission(sender, PERMISSION_VIEW)) return true;
                ...
                case "recipe" -> {
                    if (!requirePermission(sender, PERMISSION_RECIPE)) return true;
                    if (!requirePlayer(sender)) return true;
                ...
                case "clean"    -> { if (!requirePermission(sender, PERMISSION_ADMIN)) return true; handleClean(sender, args); }
                case "start"    -> { if (!requirePermission(sender, PERMISSION_ADMIN)) return true; if (requirePlayer(sender)) handleStart((Player) sender, args); }
                case "stop"     -> { if (!requirePermission(sender, PERMISSION_ADMIN)) return true; if (requirePlayer(sender)) handleStop((Player) sender, args); }
                case "reload"   -> { if (!requirePermission(sender, PERMISSION_ADMIN)) return true; handleReload(sender); }
                case "itemlist" -> { if (!requirePermission(sender, PERMISSION_ADMIN)) return true; sendItemList(sender); }
                case "give"     -> { if (!requirePermission(sender, PERMISSION_ADMIN)) return true; handleGive(sender, args); }
                default -> {
                    msg(sender, Utils.msg("prefix") + " " +"&cUnknown command. Use /sds help for a list of commands.");
                    if (sender instanceof Player p) sound(p, Sound.BLOCK_NOTE_BLOCK_BASS, 0.5f);
                }
            }
        } catch (Exception e) {
    ...
    private void sendHelpMessage(CommandSender sender) {
        ...
        if (sender instanceof Player p) sound(p, Sound.BLOCK_RESPAWN_ANCHOR_CHARGE, 1.5f);
    }
    ...
    private void handleReload(CommandSender sender) {
        try {
            ...
            for (Feature feature : Feature.values()) feature.updateConfig();
            msg(sender,  Utils.msg("prefix") + " " + "&e" + plugin.getName() + " " + plugin.getDescription().getVersion() + " reloaded.");
            if (sender instanceof Player p) sound(p, Sound.BLOCK_RESPAWN_ANCHOR_CHARGE, 1.5f);
        } catch (Exception e) {
            SuddenDeath.getInstance().getLogger().log(Level.WARNING,
    ...
    private void sendItemList(CommandSender sender) {
        ...
        for (CustomItem item : CustomItem.values()) {
            if (sender instanceof Player player) {
                Component entry = Component.text(item.getName(), NamedTextColor.WHITE)
                        .append(Component.text(" (", NamedTextColor.GRAY))
                        .append(Component.text(item.name(), NamedTextColor.WHITE)
                                .clickEvent(ClickEvent.runCommand("/sds give " + item.name()))
                                .hoverEvent(HoverEvent.showText(
                                        Component.text("Click to obtain " + item.getName()))))
                        .append(Component.text(")", NamedTextColor.GRAY));
                player.sendMessage(entry);
            } else {
                sender.sendMessage(color("&f" + item.getName() + " (" + item.name() + ")"));
            }
        }
        if (sender instanceof Player p) sound(p, Sound.BLOCK_RESPAWN_ANCHOR_CHARGE, 1.5f);
    }
    ...
    private void handleGive(CommandSender sender, String[] args) {
        ...
        if (sender != target) {
            msg(sender, Utils.msg("give-item")
                    .replace("#item#", displayName)
                    .replace("#player#", target.getName())
                    .replace("#amount#", amountStr));
        }
        msg(target, Utils.msg("receive-item")
                .replace("#item#", displayName)
                .replace("#amount#", amountStr));
