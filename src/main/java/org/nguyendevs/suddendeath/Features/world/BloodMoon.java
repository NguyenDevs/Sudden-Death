...
public class BloodMoon extends WorldEventHandler {
	...
	public void onCreatureSpawn(CreatureSpawnEvent event) {
		...
		if (!(entity instanceof Monster) || !entity.getWorld().equals(getWorld())) return;

		try {
			if (!SuddenDeath.getInstance().getWorldGuard()
					.isFlagAllowedAtLocation(entity.getLocation(), CustomFlag.SDS_EVENT)) return;
			...
			entity.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, Integer.MAX_VALUE, entitySpeed, false, false, false));
			entity.addPotionEffect(new PotionEffect(PotionEffectType.STRENGTH, Integer.MAX_VALUE, strength, false, false, false));
			entity.addPotionEffect(new PotionEffect(PotionEffectType.RESISTANCE, Integer.MAX_VALUE, resistance, false, false, false));

			if (entity instanceof Zombie zombie) applyBonusZombieTool(zombie);
		} catch (Exception e) {
	...
	public void onSkeletonArrowHit(EntityDamageByEntityEvent event) {
		...
		if (!(arrow.getShooter() instanceof AbstractSkeleton)) return;
		if (player.getGameMode() == GameMode.CREATIVE || player.getGameMode() == GameMode.SPECTATOR) return;

		try {
			if (!SuddenDeath.getInstance().getWorldGuard().isFlagAllowed(player, CustomFlag.SDS_EVENT)) return;
			if (!markedPlayers.add(player.getUniqueId())) return;
	...
	public void onCreeperFuse(ExplosionPrimeEvent event) {
		...
		try {
			if (!SuddenDeath.getInstance().getWorldGuard()
					.isFlagAllowedAtLocation(event.getEntity().getLocation(), CustomFlag.SDS_EVENT)) return;
	...
	public void onCreeperExplosion(EntityExplodeEvent event) {
		...
		try {
			if (!SuddenDeath.getInstance().getWorldGuard()
					.isFlagAllowedAtLocation(event.getLocation(), CustomFlag.SDS_EVENT)) return;
	...
	private void applyBonusZombieTool(Zombie zombie) {
		...
		if (RANDOM.nextDouble() > bonusChance) return;
		...
		if (RANDOM.nextDouble() < 0.40) applyRandomEnchant(tool);
	...
	private ToolTier selectTier() {
		int total = 0;
		for (ToolTier t : ToolTier.values()) total += t.weight;
	...
	private void applyRandomEnchant(ItemStack item) {
		...
		if (item.getType().name().contains("_AXE")) options.add(Enchantment.SMITE);
