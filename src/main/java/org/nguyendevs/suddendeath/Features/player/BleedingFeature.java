...
public class BleedingFeature extends AbstractFeature {
    ...
    private static final Set<UUID> bleedingPlayers = ConcurrentHashMap.newKeySet();

    public static void addBleedingPlayer(Player p) {
        if (p != null && p.isOnline())
            bleedingPlayers.add(p.getUniqueId());
    }

    public static void removeBleedingPlayer(Player p) {
        if (p != null)
            bleedingPlayers.remove(p.getUniqueId());
    }
                ...
                try {
                    for (UUID uuid : bleedingPlayers) {
                        ...
                        if (Feature.BLEEDING.isEnabled(player) &&
                            ...
                            Utils.damage(player, Feature.BLEEDING.getDouble("dps") * 3,
                                    Feature.BLEEDING.getBoolean("tug"));
                        }
                    }
                } catch (Exception e) {
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        ...
        if (!Feature.BLEEDING.isEnabled(player))
            return;
        if (isExcludedDamageCause(event.getCause()))
            return;
        ...
        try {
            ...
            if (plugin.getWorldGuard().isFlagAllowed(player, CustomFlag.SDS_REMOVE)) {
                if (data.isBleeding()) {
                    ...
                    removeBleedingPlayer(player);
                    player.sendMessage(ChatColor.translateAlternateColorCodes('&',
                            Utils.msg("prefix") + " " + Utils.msg("no-longer-bleeding")));
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        try {
            ...
            if (!Utils.isPluginItem(item, false))
                return;
            if (!item.isSimilar(CustomItem.BANDAGE.a()))
                return;
            PlayerData data = PlayerData.get(player);
            if (data == null || !data.isBleeding())
                return;
            ...
            removeBleedingPlayer(player);
            player.sendMessage(
                    ChatColor.translateAlternateColorCodes('&', Utils.msg("prefix") + " " + Utils.msg("use-bandage")));
            player.playSound(player.getLocation(), Sound.ITEM_ARMOR_EQUIP_LEATHER, 1.0f, 0.0f);
        } catch (Exception e) {
        }
    }
    ...
    private void applyBleeding(Player player) {
        ...
        if (RANDOM.nextDouble() <= chance && !data.isBleeding()) {
            ...
            player.sendMessage(
                    ChatColor.translateAlternateColorCodes('&', Utils.msg("prefix") + " " + Utils.msg("now-bleeding")));
    ...
    private void spawnBleedingParticles(Player player) {
        ...
        player.getWorld().spawnParticle(Particle.BLOCK_CRACK, player.getLocation().add(offsetX, offsetY, offsetZ), 30,
