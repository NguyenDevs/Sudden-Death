...
public class InfectionFeature extends AbstractFeature {
    ...
    public void onEntityRegainHealth(EntityRegainHealthEvent event) {
        if (!(event.getEntity() instanceof Player player) || player.hasMetadata("NPC"))
            return;
        if (!Feature.INFECTION.isEnabled(player))
            return;
        PlayerData data = PlayerData.get(player);
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        Player player = event.getPlayer();
        if (!event.hasItem() || !Feature.INFECTION.isEnabled(player))
            return;

        var item = player.getInventory().getItemInMainHand();
        if (!Utils.isPluginItem(item, false) || !item.isSimilar(CustomItem.STRANGE_BREW.a()))
            return;
    ...
    private void clearInfection(Player player, PlayerData data) {
        ...
        player.sendMessage(Utils.color(Utils.msg("prefix") + " " + Utils.msg("use-strange-brew")));
    ...
    private void applyInfection(Player player) {
        ...
        if (RANDOM.nextDouble() <= chance) {
            ...
            player.sendMessage(Utils.color(Utils.msg("prefix") + " " + Utils.msg("now-infected")));
