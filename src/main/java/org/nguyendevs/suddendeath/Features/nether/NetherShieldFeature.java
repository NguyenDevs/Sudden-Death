...
public class NetherShieldFeature extends AbstractFeature {
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        if (!isNetherEntity(event.getEntity())) return;
        if (!(event.getDamager() instanceof Player player)) return;
        if (!Feature.NETHER_SHIELD.isEnabled(event.getEntity())) return;
            ...
            if (RANDOM.nextDouble() <= chance) {
                ...
                for (double j = 0; j < Math.PI * 2; j += 0.3) {
                    ...
                    for (double y = 0; y < 2; y += 0.2) {
                        ...
                        if (RANDOM.nextDouble() < 0.45) loc.getWorld().spawnParticle(Particle.SMOKE_NORMAL, loc, 0);
                    }
                }
