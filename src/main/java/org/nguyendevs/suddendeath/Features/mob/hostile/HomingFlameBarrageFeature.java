...
public class HomingFlameBarrageFeature extends AbstractFeature {

    @Override
    public String getName() {
        return "Homing Flame Barrage";
    }
            ...
            public void run() {
                try {
                    for (World world : org.bukkit.Bukkit.getWorlds()) {
                        if (!Feature.HOMING_FLAME_BARRAGE.isEnabled(world)) continue;
                    ...
                    plugin.getLogger().log(Level.WARNING, "Error in HomingFlameBarrage task", e);
                }
            }
        }.runTaskTimer(plugin, 0L, 100L));
    }

    private void applyBarrage(Blaze blaze) {
