...
public class DrownedFeature extends AbstractFeature {
            ...
            public void run() {
                try {
                    for (World world : Bukkit.getWorlds()) {
                        if (!Feature.TRIDENT_WRATH.isEnabled(world)) continue;
                        for (Drowned drowned : world.getEntitiesByClass(Drowned.class)) {
                            if (drowned.getTarget() instanceof Player) applyTridentWrath(drowned);
                        }
                    }
                } catch (Exception e) {
    ...
    private void applyTridentWrath(Drowned drowned) {
        try {
            if (drowned.getHealth() <= 0) return;
            if (!(drowned.getTarget() instanceof Player target)) return;
            if (!target.getWorld().equals(drowned.getWorld())) return;
            ...
            new BukkitRunnable() {
                int ticks = 0;
                final Vector direction = target.getLocation().add(0, 1, 0)
                ...
                public void run() {
                    try {
                        ...
                        drowned.getWorld().spawnParticle(Particle.BUBBLE_COLUMN_UP,
                                drowned.getLocation().add(0, 0.5, 0), 5, 0.3, 0.3, 0.3, 0.1);

                        if (drowned.getPose() != Pose.SPIN_ATTACK) drowned.setPose(Pose.SPIN_ATTACK);

                        if (drowned.getLocation().distanceSquared(target.getLocation()) < 2.5) {
