...
public class PhantomFeature extends AbstractFeature {
    ...
    private void applyPhantomBlade(Phantom phantom) {
        ...
        try {
            if (!target.getWorld().equals(phantom.getWorld()))
                return;
            ...
            if ((now + timeOffset) - lastVanish > intervalMillis + (durationTicks * 50)) {
                phantom.addPotionEffect(
                        new PotionEffect(PotionEffectType.INVISIBILITY, (int) durationTicks, 0, false, false));
    ...
    private void shootWindBlade(Phantom phantom, Player target) {
        try {
            ...
            try {
                phantom.getWorld().playSound(startLoc, Sound.valueOf("ENTITY_BREEZE_IDLE_AIR"), 2.0f, 0.8f);
                ...
                public void run() {
                    try {
                        ...
                        for (Vector offset : bladeShape) {
                            ...
                            currentLoc.getWorld().spawnParticle(Particle.REDSTONE, pLoc, 1, 0, 0, 0, 0,
                                    new Particle.DustOptions(Color.fromRGB(180, 180, 180), 0.25f));
                        }
                        ...
                        for (Entity entity : currentLoc.getWorld().getNearbyEntities(currentLoc, 2.0, 2.0, 2.0)) {
                            if (entity instanceof Player hitPlayer) {
                                if (Utils.hasCreativeGameMode(hitPlayer))
                                    continue;
                                if (hitPlayer.getUniqueId().equals(phantom.getUniqueId()))
                                    continue;
                                Utils.damage(hitPlayer, damage, true);
                                hitPlayer.addPotionEffect(
                                        new PotionEffect(PotionEffectType.WEAKNESS, weaknessDuration, amplifier));

                                hitPlayer.getWorld().playSound(hitPlayer.getLocation(),
                                        Sound.ENTITY_PLAYER_ATTACK_SWEEP, 1.5f, 1.8f);
                                hitPlayer.getWorld().spawnParticle(Particle.SWEEP_ATTACK,
                                        hitPlayer.getLocation().add(0, 1, 0), 1, 0.5, 0.5, 0.5, 0);
                                try {
                                    Particle windParticle = Particle.valueOf("GUST");

                                    hitPlayer.getWorld().spawnParticle(windParticle,
                                            hitPlayer.getLocation().add(0, 1, 0), 2, 0.3, 0.3, 0.3, 0.1);
                        ...
                        if (currentLoc.getBlock().getType().isSolid()) {
                            currentLoc.getWorld().spawnParticle(Particle.BLOCK_CRACK, currentLoc, 15, 0.3, 0.3, 0.3,
                                    currentLoc.getBlock().getBlockData());
                            currentLoc.getWorld().playSound(currentLoc, Sound.ENTITY_ZOMBIE_ATTACK_IRON_DOOR, 1.0f,
                                    1.0f);
                            cancel();
                        }
                        ticks++;
